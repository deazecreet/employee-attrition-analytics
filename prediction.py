...
if __name__ == '__main__':
    # Memuat model
    model = load_model('submission/adaboost_model.pkl')
    
    predictions = predict_attrition('submission/test_employee.csv')
    
    # Menampilkan atau menyimpan hasil prediksi
    print(predictions)
    # Misalnya menyimpan hasil prediksi ke CSV
    pd.DataFrame(predictions, columns=['Predicted Attrition']).to_csv('submission/attrition_test_employee.csv', index=False)
